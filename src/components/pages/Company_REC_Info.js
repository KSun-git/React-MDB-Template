...
import './Company_REC_Info.css';
class Company_REC_Info extends Component {
	state= {
        isLoading: false,
		repleOn: false,
	...
	render() {
		const{ contract, memberName, implement, calculate } = this.props.location.state;
		const {isLoading, repleOn, REC_contract_Modal} = this.state;
    return (
		<div className="Company_REC_Info div_content" style={{marginTop:"70px"}}>
			
            {/* 타이틀 */}
            <MDBCard>
				<MDBCardBody className="titleCardBody">
					<span id="content_title">REC이행내역 상세/수정 </span><span style={{color:"red"}}>(권한&gt;거래소)</span>
				</MDBCardBody>
			</MDBCard>
...
export default Company_REC_Info;
